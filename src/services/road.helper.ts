...
export class RoadHelper {
  ...
  private lastY() {
    return (this.state.segments.length == 0) ? 0 : this.state.segments[this.state.segments.length - 1].p2.world.y;
  }
  ...
  private resetSprites() {
    ...
    for (n = 200; n < this.state.segments.length; n += 3) {
      this.addSprite(n, Utils.randomChoice(SPRITES.PLANTS), Utils.randomChoice([1, -1]) * (2 + Math.random() * 5));
    }
  ...
  private resetCars() {
    ...
    for (let n = 0; n < this.state.totalCars; n++) {
      ...
      speed = this.state.maxSpeed / 4 + Math.random() * this.state.maxSpeed / (sprite == SPRITES.SEMI ? 4 : 2);
