...
export class StateService {
  ...
  public update(dt: number, stateUpdater: Subject<StateUpdate>): void | StateUpdate {
    ...
    this.skyOffset = Utils.increase(
      this.skyOffset,
      (this.skySpeed * playerSegment.curve * (this.position - startPosition)) / this.segmentLength,
      1
    );
    this.hillOffset = Utils.increase(
      this.hillOffset,
      (this.hillSpeed * playerSegment.curve * (this.position - startPosition)) / this.segmentLength,
      1
    );
    this.treeOffset = Utils.increase(
      this.treeOffset,
      (this.treeSpeed * playerSegment.curve * (this.position - startPosition)) / this.segmentLength,
      1
    );
