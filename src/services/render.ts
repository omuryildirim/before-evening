...
export class Render {
  ...
  sprite(
    ...
    const destW =
      ((sprite.w * scale * this.state.width) / 2) * (SPRITES.SCALE * this.state.roadWidth);
    const destH =
      ((sprite.h * scale * this.state.width) / 2) * (SPRITES.SCALE * this.state.roadWidth);
  ...
  player(
    ...
    const bounce =
      1.5 * Math.random() * speedPercent * this.state.resolution * Utils.randomChoice([-1, 1]);
    let sprite;
  ...
  render() {
    const baseSegment = Render.findSegment(
      this.state.segments,
    ...
    const playerPercent = Utils.percentRemaining(
      this.state.position + this.state.playerZ,
      this.state.segmentLength
    );
    const playerY = Utils.interpolate(
    ...
    this.background(
      BACKGROUND.SKY,
      this.state.skyOffset,
      this.state.resolution * this.state.skySpeed * playerY
    );
    this.background(
      BACKGROUND.HILLS,
      this.state.hillOffset,
      this.state.resolution * this.state.hillSpeed * playerY
    );
    this.background(
      BACKGROUND.TREES,
      this.state.treeOffset,
    ...
    for (n = 0; n < this.state.drawDistance; n++) {
      ...
      Utils.project(
        segment.p1,
        this.state.playerX * this.state.roadWidth - x,
        playerY + this.state.cameraHeight,
        this.state.position - (segment.looped ? this.state.trackLength : 0),
        this.state.cameraDepth,
        this.state.width,
        this.state.height,
      ...
      Utils.project(
        segment.p2,
        this.state.playerX * this.state.roadWidth - x - dx,
        playerY + this.state.cameraHeight,
        this.state.position - (segment.looped ? this.state.trackLength : 0),
        this.state.cameraDepth,
        this.state.width,
      ...
      if (
        segment.p1.camera.z <= this.state.cameraDepth || // behind us
    ...
    for (n = this.state.drawDistance - 1; n > 0; n--) {
      ...
      for (i = 0; i < segment.cars.length; i++) {
        ...
        spriteX =
          Utils.interpolate(segment.p1.screen.x, segment.p2.screen.x, car.percent) +
          (spriteScale * car.offset * this.state.roadWidth * this.state.width) / 2;
      ...
      for (i = 0; i < segment.sprites.length; i++) {
        ...
        spriteX =
          segment.p1.screen.x +
          (spriteScale * sprite.offset * this.state.roadWidth * this.state.width) / 2;
