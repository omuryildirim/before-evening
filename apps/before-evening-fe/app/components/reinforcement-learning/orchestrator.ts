...
export class Orchestrator {
	...
	async run() {
		...
		const isFinished = (resolve: (value?: unknown) => void) => {
			const id = this.gameStateService.addStateUpdater(async (rawState) => {
				if (remainingSteps) {
					...
					const actionMap = this.takeAction(state, remainingSteps);
	...
	takeAction(state: tf.Tensor2D, remainingSteps: number) {
		const action = this.policyNetwork.model.chooseAction(state, this.eps);
	...
	takePredictedAction(state: tf.Tensor2D) {
		const action = this.policyNetwork.model.predictAction(state);
		let bestAction = 0;
