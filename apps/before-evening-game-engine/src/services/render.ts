...
export class Render {
	...
	sprite(
		...
		const destW =
			((sprite.w * scale * this.state.width) / 2) *
	...
	render() {
		...
		const basePercent = Utils.percentRemaining(
			this.state.position,
			this.state.segmentLength,
		);
		...
		for (n = 0; n < this.state.drawDistance; n++) {
			segment =
				this.state.segments[
					(baseSegment.index + n) % this.state.segments.length
				];
			segment.looped = segment.index < baseSegment.index;
			segment.fog = Utils.exponentialFog(
				n / this.state.drawDistance,
		...
		for (n = this.state.drawDistance - 1; n > 0; n--) {
			...
			for (i = 0; i < segment.cars.length; i++) {
				...
				spriteX =
					...
					(spriteScale * car.offset * this.state.roadWidth * this.state.width) /
						2;
				spriteY = Utils.interpolate(
					segment.p1.screen.y,
					segment.p2.screen.y,
					car.percent,
				);
				...
				this.player(
					...
					this.state.height / 2 -
						((this.state.cameraDepth / this.state.playerZ) *
							...
							2,
					this.state.speed *
						(this.state.keyLeft ? -1 : this.state.keyRight ? 1 : 0),
