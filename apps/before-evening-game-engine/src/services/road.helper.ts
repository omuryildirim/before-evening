...
export class RoadHelper {
	...
	private addRoad(
		...
		for (n = 0; n < enter; n++) {
			this.addSegment(
				Utils.easeIn(0, curve, n / enter),
				Utils.easeInOut(startY, endY, n / total),
			);
		}
		for (n = 0; n < hold; n++) {
			this.addSegment(
				curve,
				Utils.easeInOut(startY, endY, (enter + n) / total),
			);
		}
		for (n = 0; n < leave; n++) {
			this.addSegment(
	...
	public resetRoad(type?: "straight") {
		...
		for (let n = 0; n < this.state.rumbleLength; n++)
			this.state.segments[this.state.segments.length - 1 - n].color =
				COLORS.FINISH;
	...
	private resetSprites() {
		...
		for (n = 1000; n < this.state.segments.length - 50; n += 100) {
			...
			this.addSprite(
				n + Utils.randomInt(0, 50),
				Utils.randomChoice(SPRITES.BILLBOARDS),
				-side,
			);
			for (i = 0; i < 20; i++) {
	...
	private resetCars() {
		...
		for (let n = 0; n < this.state.totalCars; n++) {
			...
			z =
				Math.floor(Math.random() * this.state.segments.length) *
				this.state.segmentLength;
