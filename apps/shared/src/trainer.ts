...
import type { SaveablePolicyNetwork } from "./policy-network";
...
export const trainModelForNumberOfGames = async (params: Params) => {
	const { maxStepsPerGame, gamesPerIteration, beforeEvening, onGameEnd } =
		params;
...
const runOneEpisode = async ({
	...
	const isFinished = async (resolve) => {
		...
		while (remainingSteps) {
			...
			const epsilon =
				minEpsilon +
				(maxEpsilon - minEpsilon) * Math.exp(-LAMBDA * currentStep);
...
const takeAction = ({
	...
	if (previousAction) {
		for (const eventKey of convertActionToKeyboardKeyNumber(previousAction)) {
			beforeEvening.changeDirectionAccordingToKey(eventKey, "up");
		}
	}

	for (const eventKey of convertActionToKeyboardKeyNumber(action)) {
...
const createNewDatasetPoint = ({
	...
	for (const action of [-1, 0, 1, 2, 3, 4, 5, 6]) {
		const rawState = beforeEvening.testAction(
			convertActionToKeyboardKeyNumber(action),
		);
		const reward = ReinforcementLearningModel.computeReward(
			rawState.playerX,
			rawState.speed,
	...
	dataset.push({
		...
		selectedAction: {
			key: action,
			value: ActionKeyToEventName[action],
			epsilon,
		},
		relativeReward,
		reward,
